...
void removeKey(Lista *li, int key) {
    No *aux = li -> ini;
    int index = 0;
    while (aux != NULL) {
        if (aux -> valor == key) {
...
void add(Lista * li, int valor, int index) {
    No *no = newNo(valor);
    if (index == 0) {
        no->prox = li -> ini;
        li -> ini = no;
...
int main(){
    ...
    printLista(li);
    int pos = 2;
    printf("Removeu elemento pos. %d = %d\n",pos,remover(li,pos));
    ...
    printLista(li);
    removeKey(li,9);
    printf("Agora, após a remoção por chave\n");
    printLista(li);
}
